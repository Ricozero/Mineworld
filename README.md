...
第一阶段：服务端区块同步预算已经完成。下一步进入第二阶段：客户端区块生命周期和网格调度。

#### 第一阶段：服务端区块同步预算（已完成）

1. 每个会话继续使用 cachedVisibleChunks、coreChunks 和 pendingChunkUpdates 维护独立发送状态
2. pendingChunkUpdates 按 chunkPos 去重，使同坐标最后一次 Upsert 或 Unload 覆盖尚未发送的旧操作
3. Loading 核心区块和 Unload 优先，Upsert 按玩家距离排序
4. 每 tick 对每个会话同时限制 Upsert 消息数量和序列化后字节数，达到任一预算后保留剩余 Upsert；Unload 不占用 ChunkUpdate 预算
5. 区块发送调度从 EntitySnapshot 发送时机中分离，区块积压不能阻塞实体同步和客户端输入
6. 为待发送数量、每 tick 发送消息数、发送字节数和预算耗尽次数增加 Profiler 数据
